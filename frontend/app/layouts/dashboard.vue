<template>
  <v-app theme="tucancha">
      ...
      <v-menu location="bottom end" offset="10">
        ...
        <v-list density="comfortable" rounded="lg" min-width="220" class="pa-2">
          <div class="px-3 py-2">
            ...
            <div class="text-caption brand-muted">{{ roleLabel }}</div>
          </div>
          <v-divider class="my-1" />
          <v-list-item v-if="authStore.isBusiness" to="/business/courts" prepend-icon="mdi-plus" title="Nueva cancha" rounded="lg" />
    ...
    <!-- ── Desktop shell (sidebar) — mismo diseño para negocio y admin ── -->
    <div class="owner-shell">
      <aside class="owner-sidebar">
        <div>
          ...
          <p class="owner-brand-sub">{{ panelLabel }}</p>
        </div>

        <nav class="owner-nav">
          <NuxtLink v-for="item in navItems" :key="item.to" :to="item.to" class="owner-link">
...
const navItems = computed(() => (authStore.isBusiness ? businessNav : adminNav))
const panelLabel = computed(() => (authStore.isBusiness ? 'Panel del negocio' : 'Panel de administración'))
const roleLabel = computed(() => (authStore.isBusiness ? 'Dueño del negocio' : 'Administrador'))
